...
/**
 * the idea is: this discovers the main dataSource, and the
 * {@link TenantAwareDataSourceSupplier}, and combines them in an
 * {@link AdminDataSourceRoutingDataSource}. it will check the presence of a tenant and if
 * its there, route to the underlying tenant datasource (obtained by calling the
 * supplier), <em>or</em> the main datasource.
 */
class AdminDataSourceRoutingDataSourceBeanPostProcessor implements BeanPostProcessor, BeanFactoryAware {
	...
	public @Nullable Object postProcessAfterInitialization(Object bean, String beanName) throws BeansException {
		if (bean instanceof DataSource main) {
			...
			var tenantAwareDataSourceSupplier = (TenantAwareDataSourceSupplier) tenantId -> Objects
				.requireNonNull(supplier.get().getIfAvailable())
				.apply(tenantId);
